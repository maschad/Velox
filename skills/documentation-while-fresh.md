...
### Example 1: Real Entry from Velox Engine
...
- **Backpressure on full**: Return Err(value) instead of blocking
  - Why: Non-blocking is requirement for real-time systems
  - Alternative: Blocking with condition variable
  - Trade-off: Caller handles retry logic vs simpler API

**Challenges & Solutions**:
- **Challenge**: Understanding memory ordering guarantees on ARM vs x86
  - Context: Writing SPSC ring buffer, needed synchronization
  - Symptom: Not sure which ordering to use (Relaxed/Acquire/Release/SeqCst)
  - Root cause: ARM has weaker memory model than x86
  - Solution: Used Release on write + Acquire on read for synchronization
  - Time to fix: 30 minutes researching ARM memory model
  - Lesson: Can't rely on x86 behavior; ARM requires explicit ordering
  - Prevention: Document memory ordering choices, add tests

**Validation Results**:
- Tests: ✅ 6/6 passing (basic, full, wrap-around, FIFO, concurrent)
- Performance: ✅ 4ns (target <50ns)
- Loom tests: ✅ Passed (all interleavings checked)
- Time to complete: 2 hours
```

**Value**: 6 months later, when optimizing, I can see exactly why Release/Acquire was chosen and what alternatives were considered.

### Example 2: API Design Decision

```markdown
**Decision**: RingBuffer::push returns Result<(), T> instead of bool

**Why**:
- Caller gets value back on failure (can retry or drop)
- Type system enforces handling (can't ignore Result)
- Idiomatic Rust error handling

**Alternative considered**:
```rust
fn push(&self, value: T) -> bool  // Returns true on success
```
- Pro: Simpler signature
- Con: Value is lost on failure (caller can't retry)
- Con: Easy to ignore return value

**Trade-off**: Slightly more complex caller code (match on Result) but safer and more flexible.

**References**: Similar pattern used in std::sync::mpsc::Sender
```
...
```
Sub-agents: Produce deliverables
Documentation: Synthesizes insights
```

Example:
- Benchmark agent: Found performance targets exceeded by 12x
- Documentation: "Performance targets exceeded due to bitwise AND for modulo (avoiding division) and cache-line padding (preventing false sharing)..."
...
1. **Option A**: [Approach]
   - Pros: [Benefits]
   - Cons: [Drawbacks]
   - Cost: [Time/complexity/performance]
...
**Revisit**: [Conditions under which to reconsider]
```

### For Performance Optimization
```markdown
**Optimization**: [What we optimized]

**Before**: [Measurement before]
**After**: [Measurement after]
**Improvement**: [X% or Xx faster]

**Technique**: [What we changed]

**Why it worked**: [Explanation]

**Trade-offs**: [What we sacrificed, if anything]

**Applicable to**: [Where else this technique applies]
```
...
**Related Issues**: [Similar bugs elsewhere]
```

## Deliverables

At project completion, you'll have:

1. **CLAUDE.md** - Complete development narrative
2. **CLAUDE_SKILLS.md** - Reusable patterns extracted
3. **README.md** - User-facing documentation
4. **ARCHITECTURE.md** - System design (from sub-agent)
5. **BENCHMARKS.md** - Performance validation (from sub-agent)
6. **SAFETY_AUDIT.md** - Security analysis (from sub-agent)

Total documentation: ~100KB of markdown
Time invested: ~5% of project time
Value: Immeasurable (reference for years)

## References

- Real example: Velox Engine CLAUDE.md (62KB)
- Captured 7 phases, 10+ challenges, 8 key patterns
- Written incrementally over 2 hours
- Became foundation for CLAUDE_SKILLS.md
- Used to onboard others to lock-free programming concepts
