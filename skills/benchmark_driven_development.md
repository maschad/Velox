...
### Example 1: Velox OrderBook Optimization

**Requirement**: Handle 100k updates/sec with CAS-based concurrency

**Iteration 1: Naive CAS loop**
```rust
// Unbounded retry
pub fn update_bid(&self, price: u32, size_delta: i64) -> Result<(), OrderBookError> {
    let idx = (price >> 4) as usize;
    loop {
        let current = self.levels[idx].load(Ordering::Relaxed);
        let new_size = (current as i64 + size_delta).max(0) as u64;

        if self.levels[idx]
            .compare_exchange(current, new_size, Ordering::Release, Ordering::Relaxed)
            .is_ok()
        {
            return Ok(());
        }
        // Infinite loop if contention is high!
    }
}
```

**Benchmark result**: P99 = 45μs (FAILED - target was <10μs)

**Iteration 2: Add exponential backoff**
```rust
pub fn update_bid(&self, price: u32, size_delta: i64) -> Result<(), OrderBookError> {
    let idx = (price >> 4) as usize;
    let mut backoff = 1;
    ...
    loop {
        ...
        match self.levels[idx].compare_exchange_weak(
            current, new_size, Ordering::Release, Ordering::Relaxed
        ) {
            Ok(_) => return Ok(()),
            Err(_) => {
                for _ in 0..backoff {
...
**Benchmark result**: P99 = 12μs (still FAILED)

**Iteration 3: Add bounded retry**
```rust
pub fn update_bid(&self, price: u32, size_delta: i64) -> Result<(), OrderBookError> {
    let idx = (price >> 4) as usize;
    let mut backoff = 1;

    for _ in 0..MAX_RETRIES {  // Bound at 100 attempts
        let current = self.levels[idx].load(Ordering::Relaxed);
        let new_size = (current as i64 + size_delta).max(0) as u64;
...
**Benchmark result**: P99 = 8μs (PASSED ✅)

**Final validation**:
- ✅ P99 latency: 8μs (under 10μs target)
- ✅ Throughput: 125k updates/sec (over 100k target)
- ✅ Timeout rate: <0.1% under normal load

### Example 2: Velox Telemetry Overhead Budget

**Requirement**: Telemetry overhead must be <5% of baseline throughput

**Baseline (no telemetry)**:
```bash
cargo bench -- transaction_processing_baseline --save-baseline no_telemetry
# Result: 900ns per transaction
```
...
```bash
cargo bench -- --baseline no_telemetry
# Result: 945ns per transaction
# Overhead: (945 - 900) / 900 = 5.0%
```

**Decision**: Overhead exactly at 5% budget → APPROVED ✅
...
### ✅ Indicators working:
- All benchmarks show statistically significant improvements
- Requirements are met (quantified targets achieved)
- Flamegraph confirms hot path is optimized
- Optimizations are localized (1-2 functions changed per iteration)
- Team can explain *why* optimization worked (data-driven)
- Benchmark suite runs in CI and catches regressions

### ❌ Indicators failing:
- "Optimizations" show no improvement or make things worse
- Optimization changed 50% of codebase (unfocused)
- Can't explain why optimization worked (luck/guessing)
- Requirements still not met after multiple iterations (wrong approach)
- Benchmarks are flaky (>20% variance across runs)
- Team is optimizing cold paths that don't matter

## Related skills

- [Performance Profiling in Rust](performance_profiling_rust.md) - Set up profiling tools
- [Cache-Line Optimization](cache_line_optimization.md) - Specific optimization technique
- [Latency Measurement](latency_measurement.md) - Measure sub-microsecond latencies
- [Incremental Validation](incremental-validation.md) - Test after each optimization

## Critical files to reference

From Velox Engine:
- `benches/telemetry_overhead.rs` (162 lines) - Overhead budget validation
- `benches/orderbook_bench.rs` (180 lines) - Multi-threaded optimization
- `src/orderbook.rs` (lines 49-62) - Bounded CAS with backoff
- `CLAUDE.md` (Phase 3 "Lock-Free Order Book") - Optimization iterations documented

## Case Study: Velox Engine Development

**Starting point**: Requirements defined (100k txn/sec, P99 <10μs)

**Phases**:

1. **Phase 2**: Naive SPSC ring buffer
   - Benchmark: 100ns/op
   - Profiling: Cache misses on head/tail
   - Optimization: Cache-line padding
   - Result: 18ns/op (5.5x faster) ✅

2. **Phase 3**: Naive OrderBook CAS
   - Benchmark: P99 = 45μs
   - Profiling: Contention on single price level
   - Optimization: Exponential backoff
   - Result: P99 = 12μs (3.75x faster) ❌ (still too slow)
   - Further optimization: Bounded retry
   - Result: P99 = 8μs (5.6x faster) ✅

3. **Phase 8**: Telemetry instrumentation
   - Benchmark baseline: 900ns/txn
   - Budget: <5% overhead (45ns)
   - Implementation: Stage-boundary instrumentation only
   - Result: 945ns/txn (5.0% overhead) ✅

**Total time**: ~2 hours (including all iterations)

**Outcome**: All performance requirements exceeded

## References

- [Velox Engine CLAUDE.md](../CLAUDE.md) - Full development log with benchmark results
- [Criterion.rs Book](https://bheisler.github.io/criterion.rs/book/getting_started.html)
- [Rust Performance Book](https://nnethercote.github.io/perf-book/)
- [Papers We Love: Performance](https://github.com/papers-we-love/papers-we-love/tree/master/performance)
