...
```markdown
1. Core Types (fixed-size, repr(C))
2. Memory Allocators (stack/arena)
3. Lock-Free Data Structures
4. Worker Threads + Affinity
5. Pipeline Wiring
6. Testing (unit, property, stress)
...
```markdown
1. HAL Abstraction (hardware interface)
2. Drivers (peripherals)
3. RTOS Tasks (if applicable)
4. Application Logic
5. Power Management
6. Testing (unit + hardware-in-loop)
7. Profiling (memory, timing, power)
```

## References

- This skill was extracted from the Velox Engine project (lock-free HFT pipeline)
- Planning saved ~8 hours of refactoring time
- All 7 phases completed with zero architectural changes
- Performance targets exceeded by 12-69x
