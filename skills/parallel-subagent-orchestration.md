...
### Example 1: Validating a Project (Real - Velox Engine)
...
**Token usage:**
- 4 agents × 20K tokens each = 80K tokens
- But 3.5x faster = more productive hour
- Trade-off: Spend tokens to save human time

**When to optimize:**
- **Don't optimize** if you're under budget and time-constrained
- **Do optimize** if approaching token limits

**Optimization strategies:**
1. Use `model: "haiku"` for simpler agents
2. Be more specific to reduce exploration
3. Reduce number of concurrent agents

## Troubleshooting

### Problem: Agents returning similar results
**Cause:** Prompts too similar or overlapping
**Solution:** Make tasks more distinct, check for overlap

### Problem: One agent much slower than others
**Cause:** Task complexity mismatch
**Solution:** Break slow task into sub-tasks for next iteration

### Problem: Conflicting results
**Cause:** Different assumptions or incomplete prompts
**Solution:** Review prompts for ambiguity, add constraints

## References

- Real example: Velox Engine project validation
- 4 agents launched in parallel
- Completed in 7 minutes vs 25+ sequential
- Benchmark agent discovered real bug
