...
**Full implementation** from Velox:
```rust
#[repr(C, align(64))]
struct CachePadded<T> {
    value: T,
...
### Step 6: Apply Struct-Level Alignment

For entire structs that should not share cache lines with neighbors:

```rust
/// OrderBook with cache-line aligned price levels
#[repr(C, align(64))]
pub struct OrderBook {
    levels: [AtomicU64; 1024],  // Each level is a price bucket
    best_bid: CachePadded<AtomicU32>,
    best_ask: CachePadded<AtomicU32>,
}

// Ensures entire OrderBook starts on cache-line boundary
const_assert_eq!(std::mem::align_of::<OrderBook>(), 64);
```

**When to use struct-level alignment**:
- Struct contains multiple hot atomics
- Struct will be in an array accessed by multiple threads
- Want to guarantee struct doesn't share cache line with adjacent data
...
### Example 1: Velox SPSC Ring Buffer

From `src/ring.rs` (lines 7-17):
...
**Impact**: Without padding, SPSC throughput was 10M ops/sec. With padding, achieved 55M ops/sec (5.5x faster).

### Example 2: Velox OrderBook Alignment

From `src/orderbook.rs` (lines 32-39):

```rust
#[repr(C, align(64))]
pub struct OrderBook {
    /// 1024 price levels (bucketed by price >> 4)
    levels: [AtomicU64; 1024],

    /// Best bid/ask tracking (relaxed updates)
    best_bid: CachePadded<AtomicU32>,
    best_ask: CachePadded<AtomicU32>,
}
```

**Why struct-level alignment**:
- Ensures entire OrderBook doesn't share cache line with adjacent data
- Critical when OrderBook is part of larger struct or array
- Prevents false sharing from unexpected neighbors

### Example 3: Velox Latency Histogram

From `src/histogram.rs` (lines 10-28):
...
**Memory footprint**:
- Without padding: 13 * 8 = 104 bytes
- With padding: 13 * 64 + 4 * 64 = 1088 bytes (10x larger)
- **Trade-off**: 10x memory for 20-50x performance in concurrent scenario

### Example 4: Power-of-Two Struct Layout

From `src/types.rs` (lines 8-24):

```rust
#[repr(C)]
pub struct Transaction {
    pub id: u64,           // 8 bytes
    pub price: u32,        // 4 bytes
    pub size: u16,         // 2 bytes
    pub flags: u8,         // 1 byte
    _padding1: u8,         // 1 byte (manual)
    pub ingress_ts_ns: u64, // 8 bytes
    _padding2: [u8; 8],    // 8 bytes (manual)
}
// Total: 32 bytes (power of 2, cache-friendly)

const_assert_eq!(std::mem::size_of::<Transaction>(), 32);
```

**Why power-of-two sizes**:
- Fits evenly in cache lines (64 bytes = 2 transactions)
- Fast array indexing (no multiplication needed)
...
### ✅ Indicators working:
- Multi-threaded benchmark shows 5-50x improvement
- Profiling shows reduced cache miss rate
- `perf stat` shows lower LLC-load-misses
- Scalability improves (2 cores → 2x throughput)
- P99 latency becomes more consistent

### ❌ Indicators failing:
- Benchmark shows no improvement (no false sharing to fix)
- Memory usage increased but performance didn't
- Alignment verification fails (`const_assert_eq!` error)
- Profiling still shows high cache miss rate (wrong thing padded)
- Code became more complex without measurable benefit

## Related skills

- [Performance Profiling in Rust](performance_profiling_rust.md) - Detect false sharing with profiling
- [Benchmark-Driven Development](benchmark_driven_development.md) - Measure impact of padding
- [Latency Measurement](latency_measurement.md) - Track latency improvements

## Critical files to reference

From Velox Engine:
- `src/ring.rs` (lines 7-17, 32-40) - SPSC ring buffer padding
- `src/orderbook.rs` (lines 32-39, 49-62) - Struct-level alignment
- `src/histogram.rs` (lines 10-28) - Array of padded atomics
- `src/types.rs` (lines 8-24) - Power-of-two layout
- `CLAUDE.md` ("Key Patterns Developed" #1) - Cache-line padding rationale

## References

- [Preshing: False Sharing](https://preshing.com/20120515/memory-reordering-caught-in-the-act/)
- [Rust Atomics and Locks Book](https://marabos.nl/atomics/hardware.html) - Chapter on cache coherency
- [Intel Optimization Manual](https://www.intel.com/content/www/us/en/architecture-and-technology/64-ia-32-architectures-optimization-manual.html) - Section on cache line size
- [ARM Memory Model Documentation](https://developer.arm.com/documentation/102336/0100)
- [Linux Kernel: `__cacheline_aligned`](https://elixir.bootlin.com/linux/latest/source/include/linux/cache.h)
